    ...
    def run(self, train_loder, val_loder, test_loder):
        ...
        if start_epoch != 0:
            ...
            if os.path.isfile(model_path):
                ...
                # 恢复 best_err（如果存在）
                best_err1 = checkpoint.get('best_err1', 100)
                best_err5 = checkpoint.get('best_err5', 100)
                # 恢复 iter 计数器（用于 tensorboard）
                self.iter = start_epoch * len(train_loder)
                # 恢复 scheduler 状态
                for _ in range(self.iter):
                    self.scheduler.step()
                print(f"=> Resumed from epoch {start_epoch}, best_err1: {best_err1:.4f}, best_err5: {best_err5:.4f}")
            else:
        ...
        for e in range(hp.epochs):
            ...
            # 阶段性保存 checkpoint（每 save_epoch_freq 个 epoch）
            if e % hp.save_epoch_freq == 0:
                ckpt_path = os.path.join(hp.model_save, f'{hp.model_name}_epoch_{e}.pkl')
                save_json = {
                    'net_state_dict': self.model.module.state_dict() if len(hp.gpus) > 1 else self.model.state_dict(),
                ...
                torch.save(save_json, ckpt_path)
